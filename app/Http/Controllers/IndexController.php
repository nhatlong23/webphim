...
class IndexController extends Controller
{
    public function home(Request $request)
    {
        ...
        $meta_image = $info->logo;
        if (substr($meta_image, 0, 4) !== 'http') {
            $meta_image = url('uploads/logo/' . $meta_image);
        }
        $sub30days = Carbon::now('Asia/Ho_Chi_Minh')->subDays(30);
        $movie_hot = Movie::where('movie_hot', 1)->where('status', 1)->where('date_created', '>=', $sub30days)->inRandomOrder()->take(10)->get();
    ...
    public function search(Request $request)
    {
        ...
        })->where('status', 1)->orderBy('date_created', 'DESC')->paginate(30);
    ...
    public function contact()
    {
        ...
        return view('pages.info.contact', compact('meta_title', 'meta_description', 'meta_image', 'contact','info'));
    }
